...
function preload() {
	// Load the image
	img = loadImage("botxo-sqlogo-bw.jpg")
}
...
/*	
  ...
  */
	...
	picture = new Picture3D(img, 5, 4, 0.7, false)
	// Changing the built in modX() function so the x value moves it across the screen, randomized using the number of the rectangle
	picture.modX = function(Rect) {
		let v1 = Rect.number - 400 >= 0 ? -10 : 10
		//let value = this.resX - Math.pow(tan(noise(Rect.number)/1- frameCount/75), 3)*v1
		let value = this.resX - tan((Rect.number/50)/20 - frameCount*0.01)*v1
		return (value / 2)
	}
}
...
function draw() {
  ...
  pointLight(255, 255, 255, locX, locY, 20);
	background(25,10);
	specularMaterial(250,25,25);
...
p5.prototype.Word3D = function(string, depth, size, resolution, bevelled = true, font = "IBM Plex", style = BOLD) {
